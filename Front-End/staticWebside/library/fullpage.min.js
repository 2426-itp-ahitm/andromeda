...
document.addEventListener('DOMContentLoaded', function () {
    ...
    new fullpage('#fullpage', {
        ...
        onLeave: function (origin, destination, direction) {
            const totalSections = sections.length - 1; // Index der letzten Section
            const percentage = destination.index / totalSections;

            // Setze Position der Rakete basierend auf Scrollstatus
            const top = 10 + percentage * 90; // Zwischen 10% und 100%
            const left = 10 + percentage * 80; // Bewegung nach rechts

            // Raketenposition aktualisieren
            rocket.style.top = `${top}%`;
            rocket.style.left = `${left}%`;
            rocket.style.transform = `rotate(${360 * percentage}deg)`; // Drehe die Rakete
        },
    });
});
